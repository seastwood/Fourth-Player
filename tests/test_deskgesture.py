"""The gestures a finger has instead of buttons, in a real browser.

Two fingers for the right button, and tap-then-tap-and-stay for holding the
left one down through a drag. Both are decided from the same stream of
pointer events as a plain tap and a pinch, and the first version of them read
the previous gesture's leftovers -- two fingers became a double tap, and the
tap after that a right click.
"""
import os
import shutil
import subprocess
...
try:
    ...
    result = subprocess.run(
        [node, os.path.join(HERE, "browser", "deskgesture.mjs")],
