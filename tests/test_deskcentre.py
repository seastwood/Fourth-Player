"""Where the pointer sits on the screen, in a real browser.

The rule is one sentence -- the pointer stays in the middle of what can be
*seen* -- and the arithmetic behind it was wrong three separate times. Every
one of them read as correct and was only caught by putting numbers through it:
measuring the element after its own transform, allowing an upward shift that
was right only for pictures taller than the screen, and stopping the picture
at its edges on a phone where it never reaches them.
"""
import os
import shutil
import subprocess
...
try:
    ...
    result = subprocess.run(
        [node, os.path.join(HERE, "browser", "deskcentre.mjs")],
