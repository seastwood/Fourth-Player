"""Input-event codes, generated from evdev -- do not edit by hand.

Regenerate with tools/gencodes.py on a Linux machine that has evdev.
"""

# name -> code, exactly as the kernel defines them. These are a stable ABI,
# so the numbers are the same on every Linux and are safe to carry to a
# machine that has no kernel to ask -- see codes.py for why that matters.
CODES = {
    'ABS_BRAKE': 10,
    'ABS_CNT': 64,
    'ABS_DISTANCE': 25,
...
# code -> name, for the one place that asks the question backwards
# (overlay.py, naming the key somebody just pressed).
KEY = {
    0: 'KEY_RESERVED',
    1: 'KEY_ESC',
    2: 'KEY_1',
